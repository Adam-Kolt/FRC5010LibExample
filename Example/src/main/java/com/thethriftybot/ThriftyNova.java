...
public final class ThriftyNova extends CANDevice {
  ...
  private abstract static class SubConfig {
  ...
  private void addError(Error err) {
    if (errors.size() < 256) errors.add(err);
  }
  ...
  public boolean hasError(Error err) {
    for (Error e : errors) if (e == err) return true;
    return false;
  }
  ...
  /**
   * Sets the time to ramp up in seconds from 0 to 10. For example, an input of 0.5 will ramp the
   * motor from idle to 100% over the course of 0.5 seconds.
   *
  ...
  /**
   * Sets the time to ramp down in seconds from 0 to 10. For example, an input of 0.5 will ramp the
   * motor from 100% to idle over the course of 0.5 seconds.
   *
  ...
  /**
   * Set the max current the motor can draw in amps. Motor speed will be capped to satisfy the max
   * current. Also set the what current reading is used for limiting calculations, between: -
   * Stator: Uses the total draw of phase a, b, and c. - Supply: Uses stator plus the draw of the
   * controller itself.
   *
  ...
  /**
   * Sets the percent output of the motor from between 1.0 (full forward) and -1.0 (full reverse),
   * where 0.0 is full stop.
   *
  ...
  /**
   * Returns the last set point specified by the user in any control method. Percent output set
   * point becomes zero.
   *
  ...
  /**
   * Returns the error to the last set point that was input, based on last used control mode
   * (position or velocity), and encoder type (internal or quaderature).
   *
  ...
  private void setConfig(int index, int value, Error err) {
    if (!setMotorConfig(index, value)) addError(err);
  }
  ...
  private boolean setMotorConfig(int index, int value) {
    ...
    // Checks if robot disabled because 1. blocking isn't as much of an issue and 2. rio start up
    // times
    while ((RobotController.getFPGATime() - rioTime) < (RobotState.isDisabled() ? 50_000 : 5_000)) {
      CANAPIJNI.readCANPacketNew(deviceHandle, APIClass.STATUS.get(5), dataBuffer);
